...
// *** Typ für entfaltete Termine (wie in termine-bearbeiten) ***
...
export default function DashboardPage() {
    ...
    const exceptionsRef = useMemoFirebase(
        ...
        [firestore, isAdmin] // Abhängig von isAdmin
    );
    ...
    const unrolledAppointments = useMemo(() => {
        // *** Warte auf `appointments` und `exceptions` (falls Admin) ODER nur auf `appointments` (falls kein Admin) ***
        if (!appointments || (isAdmin && isLoadingExceptions)) return []; 
        
        const exceptionsMap = new Map<string, AppointmentException>();
        // `exceptions` ist nur für Admins verfügbar, sonst null
        ...
        appointments.forEach(app => {
            ...
            const exception = exceptionsMap.get(key); // Wird `undefined` sein, wenn kein Admin
            ...
            if (app.recurrence === 'none') {
                ...
                while (currentDate < recurrenceEndDate && iter < MAX_ITERATIONS) {
                    ...
                    const instanceException = exceptionsMap.get(instanceKey); // `undefined` wenn kein Admin
    ...
    }, [appointments, exceptions, isLoadingExceptions, userTeamIds, isAdmin]); // isAdmin als Abhängigkeit


    const isLoading = isUserLoadingAuth || isLoadingMember || isLoadingAppointments || (isAdmin && isLoadingExceptions) || isLoadingNews || isLoadingPollsAll || isLoadingPollsTeams || isLoadingGroups;
