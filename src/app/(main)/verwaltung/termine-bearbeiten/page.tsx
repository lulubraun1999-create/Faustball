...
const singleAppointmentInstanceSchema = z
  ...
  .refine((data) => !data.endDate || !data.startDate || data.endDate >= data.startDate, { message: 'Enddatum muss nach dem Startdatum liegen.', path: ['endDate'] });
...
const useAppointmentSchema = (appointmentTypes: AppointmentType[] | null) => {
  return useMemo(() => {
    ...
    return z
      ...
      .refine((data) => data.isAllDay || !data.endDate || !data.startDate || data.endDate >= data.startDate, { path: ['endDate'], message: 'Enddatum muss nach dem Startdatum liegen.' })
      ...
      .refine((data) => data.recurrence === 'none' || !!data.recurrenceEndDate, { message: 'Enddatum für Wiederholung ist erforderlich.', path: ['recurrenceEndDate'] })
      .refine(data => { if(data.recurrence !== 'none' && data.recurrenceEndDate && data.startDate) { try { const start = new Date(data.startDate); const end = new Date(data.recurrenceEndDate); return end >= start; } catch(e) { return false; } } return true; }, { message: 'Ende der Wiederholung muss nach dem Startdatum liegen.', path: ['recurrenceEndDate'] })
      .superRefine((data, ctx) => { if (data.appointmentTypeId === sonstigeTypeId && (!data.title || data.title.trim() === '')) { ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Titel ist bei Typ "Sonstiges" erforderlich.', path: ['title'] }); } });
  }, [appointmentTypes]);
};
...
export default function AdminTerminePage() {
  ...
  const [pendingUpdateData, setPendingUpdateData] = useState<SingleAppointmentInstanceFormValues | null>(null);
  const [teamFilter, setTeamFilter] = useState<string>('all');
  const [typeFilter, setTypeFilter] = useState<string>('all');

  // Data Fetching
  const appointmentsRef = useMemoFirebase(() => (firestore && isAdmin ? collection(firestore, 'appointments') : null), [firestore, isAdmin]);
  ...
  const isLoading = isUserLoading || isLoadingAppointments || isLoadingTypes || isLoadingLocations || isLoadingGroups || isLoadingExceptions;

  // Memoized Data Processing
  const { typesMap, locationsMap, teams, teamsMap, groupedTeams, sortedTeamsForFilter } = useMemo(() => {
    if (!appointmentTypes || !locations || !groups) return { typesMap: new Map(), locationsMap: new Map(), teams: [], teamsMap: new Map(), groupedTeams: [], sortedTeamsForFilter: [] };
    ...
    const customSort = (a: Group, b: Group) => {
        const regex = /^(U|u)(\d+)/; const matchA = a.name.match(regex); const matchB = b.name.match(regex);
        if (matchA && matchB) { return parseInt(matchA[2], 10) - parseInt(matchB[2], 10); }
        if (matchA) return -1; if (matchB) return 1; return a.name.localeCompare(b.name);
    };
    const sortedTeams = [...teams].sort(customSort);
    const grouped = classes.map((c) => ({ ...c, teams: teams.filter((t) => t.parentId === c.id).sort(customSort) })).filter((c) => c.teams.length > 0);
    return { typesMap, locationsMap, teams, teamsMap, groupedTeams: grouped, sortedTeamsForFilter: sortedTeams };
  ...
  const filteredAndGroupedAppointments = useMemo(() => {
    ...
    const filtered = unrolledAppointments.filter(app => {
        ...
        return isVisibleByTeam && isVisibleByType;
    });

    const sorted = filtered.sort((a, b) => a.startDate.toMillis() - b.startDate.toMillis());
  ...
  // Forms and Dialog Logic
  const appointmentSchema = useAppointmentSchema(appointmentTypes);
  const appointmentForm = useForm<AppointmentFormValues>({ resolver: zodResolver(appointmentSchema), defaultValues: { title: '', appointmentTypeId: '', startDate: '', endDate: '', isAllDay: false, recurrence: 'none', recurrenceEndDate: '', visibilityType: 'all', visibleTeamIds: [], rsvpDeadline: '', locationId: '', meetingPoint: '', meetingTime: '', description: '' } });
  ...
  const onSubmitAppointment = async (data: AppointmentFormValues) => {
    ...
    const finalTitle = data.appointmentTypeId !== sonstigeTypeId && (!data.title || data.title.trim() === '') ? selectedTypeName : data.title?.trim();
    if (data.appointmentTypeId === sonstigeTypeId && (!finalTitle || finalTitle.trim() === '')) { appointmentForm.setError('title', { message: 'Titel ist bei Typ "Sonstiges" erforderlich.' }); setIsSubmitting(false); return; }
    const startDate = new Date(data.startDate); const endDate = data.endDate ? new Date(data.endDate) : null; const rsvpDeadline = data.rsvpDeadline ? new Date(data.rsvpDeadline) : null; const recurrenceEndDate = data.recurrenceEndDate ? new Date(data.recurrenceEndDate) : null;
    ...
    const appointmentData: Omit<Appointment, 'id' | 'lastUpdated'> = { title: finalTitle || '', appointmentTypeId: data.appointmentTypeId, startDate: startDateTimestamp, ...(endDateTimestamp && !data.isAllDay && { endDate: endDateTimestamp }), isAllDay: data.isAllDay, recurrence: data.recurrence, ...(recurrenceEndDateTimestamp && data.recurrence !== 'none' && { recurrenceEndDate: recurrenceEndDateTimestamp }), visibility: { type: data.visibilityType, teamIds: data.visibilityType === 'specificTeams' ? data.visibleTeamIds : [] }, ...(rsvpDeadlineTimestamp && { rsvpDeadline: rsvpDeadlineTimestamp }), ...(data.locationId && { locationId: data.locationId }), ...(data.meetingPoint && { meetingPoint: data.meetingPoint }), ...(data.meetingTime && { meetingTime: data.meetingTime }), ...(data.description && { description: data.description }), createdBy: selectedAppointment?.createdBy || user.uid, createdAt: selectedAppointment?.createdAt || serverTimestamp() };
    try {
      if (selectedAppointment) { const docRef = doc(firestore, 'appointments', selectedAppointment.id); await updateDoc(docRef, { ...appointmentData, lastUpdated: serverTimestamp() }); toast({ title: 'Terminserie erfolgreich aktualisiert.' }); } else { const appointmentsColRef = collection(firestore, 'appointments'); await addDoc(appointmentsColRef, { ...appointmentData }); toast({ title: 'Neuer Termin erfolgreich erstellt.' }); }
      resetAppointmentForm(); setIsAppointmentDialogOpen(false);
    } catch (error: any) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: selectedAppointment ? `appointments/${selectedAppointment.id}` : 'appointments', operation: selectedAppointment ? 'update' : 'create', requestResourceData: appointmentData })); } finally { setIsSubmitting(false); }
  };
  const onSubmitSingleInstance = async (data: SingleAppointmentInstanceFormValues) => {
    if (!selectedInstanceToEdit) return;
    const cleanData = { ...data, endDate: data.endDate || null };
    setPendingUpdateData(cleanData); setIsInstanceDialogOpen(false); setIsUpdateTypeDialogOpen(true);
  };
  const resetSingleInstanceDialogs = () => { setIsInstanceDialogOpen(false); setIsUpdateTypeDialogOpen(false); setSelectedInstanceToEdit(null); setPendingUpdateData(null); instanceForm.reset(); };
  async function handleSaveSingleOnly() {
    if (!firestore || !pendingUpdateData || !selectedInstanceToEdit || !user) return;
    setIsSubmitting(true);
    try { const { firebaseApp } = initializeFirebase(); const functions = getFunctions(firebaseApp); const saveSingleException = httpsCallable(functions, 'saveSingleAppointmentException'); await saveSingleException(pendingUpdateData); toast({ title: "Erfolg", description: "Die Terminänderung wurde gespeichert." });
    } catch (error: any) { toast({ variant: "destructive", title: "Fehler beim Speichern", description: error.message || "Die Änderung konnte nicht gespeichert werden." }); } finally { setIsSubmitting(false); resetSingleInstanceDialogs(); }
  }
  async function handleSaveForFuture() {
    if (!firestore || !pendingUpdateData || !selectedInstanceToEdit || !user) return;
    setIsSubmitting(true); if (!isAdmin) { toast({ variant: "destructive", title: "Fehler", description: "Nur Administratoren können diese Aktion ausführen." }); setIsSubmitting(false); return; }
    try { const { firebaseApp } = initializeFirebase(); const functions = getFunctions(firebaseApp); const saveFutureInstancesFn = httpsCallable(functions, 'saveFutureAppointmentInstances'); const payload = { originalId: selectedInstanceToEdit.originalId, ...pendingUpdateData }; await saveFutureInstancesFn(payload); toast({ title: 'Terminserie erfolgreich aufgeteilt und aktualisiert' });
    } catch (error: any) { console.error('Error splitting and saving future instances: ', error); toast({ variant: "destructive", title: "Fehler beim Speichern", description: error.message || "Die Terminserie konnte nicht aktualisiert werden." }); } finally { setIsSubmitting(false); resetSingleInstanceDialogs(); }
  }
  const handleCancelSingleInstance = async (appointment: UnrolledAppointment) => {
    ...
    } catch (error: any) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'appointmentExceptions', operation: 'write' })); } finally { setIsSubmitting(false); }
  };
  const handleDeleteAppointment = async (id: string) => {
    if (!firestore) return; setIsSubmitting(true);
    try { const batch = writeBatch(firestore); const appointmentDocRef = doc(firestore, 'appointments', id); const exceptionsQuery = query(collection(firestore, 'appointmentExceptions'), where('originalAppointmentId', '==', id)); const exceptionsSnapshot = await getDocs(exceptionsQuery); exceptionsSnapshot.forEach((doc) => batch.delete(doc.ref)); batch.delete(appointmentDocRef); await batch.commit(); toast({ title: 'Terminserie und alle Ausnahmen gelöscht' });
    } catch (e: any) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: `appointments/${id}`, operation: 'delete' })); } finally { setIsSubmitting(false); }
  };
  const handleEditAppointment = (appointment: UnrolledAppointment) => {
    if (!appointments) return;
    const originalAppointment = appointments.find(app => app.id === appointment.originalId); if (!originalAppointment) return;
    ...
    const typeName = typesMap.get(appointment.appointmentTypeId); const isSonstiges = typeName === 'Sonstiges'; const titleIsDefault = !isSonstiges && appointment.title === typeName;
    instanceForm.reset({ originalDateISO: appointment.originalDateISO, startDate: startDateString, endDate: endDateString, isAllDay, title: titleIsDefault ? '' : appointment.title, locationId: appointment.locationId ?? '', meetingPoint: appointment.meetingPoint ?? '', meetingTime: appointment.meetingTime ?? '', description: appointment.description ?? '' });
    setIsInstanceDialogOpen(true);
  };
  const resetAppointmentForm = () => { appointmentForm.reset({ title: '', appointmentTypeId: '', startDate: '', endDate: '', isAllDay: false, recurrence: 'none', recurrenceEndDate: '', visibilityType: 'all', visibleTeamIds: [], rsvpDeadline: '', locationId: '', meetingPoint: '', meetingTime: '', description: '' }); setSelectedAppointment(null); };
  const onSubmitAppointmentType = async (data: AppointmentTypeFormValues) => {
    if (!firestore) return; const typesColRef = collection(firestore, 'appointmentTypes');
    try { await addDoc(typesColRef, { name: data.name }); toast({ title: 'Termin-Art erfolgreich erstellt.' }); typeForm.reset();
    } catch (e) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'appointmentTypes', operation: 'create', requestResourceData: data })); }
  };
  const onDeleteAppointmentType = async (id: string) => {
    if (!firestore) return; const docRef = doc(firestore, 'appointmentTypes', id);
    try { await deleteDoc(docRef); toast({ title: 'Termin-Art gelöscht.' });
    } catch (e) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: `appointmentTypes/${id}`, operation: 'delete' })); }
  };
  const onSubmitLocation = async (data: LocationFormValues) => {
    if (!firestore) return; const locationsColRef = collection(firestore, 'locations');
    try { await addDoc(locationsColRef, { name: data.name, address: data.address }); toast({ title: 'Ort erfolgreich erstellt.' }); locationForm.reset();
    } catch (e) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'locations', operation: 'create', requestResourceData: data })); }
  };
  const onDeleteLocation = async (id: string) => {
    if (!firestore) return; const docRef = doc(firestore, 'locations', id);
    try { await deleteDoc(docRef); toast({ title: 'Ort gelöscht.' });
    } catch (e) { errorEmitter.emit('permission-error', new FirestorePermissionError({ path: `locations/${id}`, operation: 'delete' })); }
  };

  // Render Logic
  if (isLoading) { return <div className="flex h-[calc(100vh-200px)] w-full items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>; }
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <Dialog open={isAppointmentDialogOpen} onOpenChange={(open) => { setIsAppointmentDialogOpen(open); if (!open) resetAppointmentForm(); }}>
        <DialogContent className="sm:max-w-2xl flex flex-col max-h-[90vh]">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><CalendarPlus className="h-5 w-5" />{selectedAppointment ? 'Termin bearbeiten' : 'Termin hinzufügen'}</DialogTitle><DialogDescription>{selectedAppointment ? 'Details des Termins ändern.' : 'Neuen Termin oder Serie hinzufügen.'}</DialogDescription></DialogHeader>
            <Form {...appointmentForm}><form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); }} className="space-y-4 flex-grow overflow-hidden flex flex-col"><ScrollArea className="flex-grow pr-6 -mr-6"><div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FormField control={appointmentForm.control} name="recurrence" render={({ field }) => ( <FormItem><FormLabel>Wiederholung</FormLabel><Select onValueChange={(value) => { field.onChange(value); if (value === 'none') { appointmentForm.setValue('recurrenceEndDate', ''); } }} value={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="none">Keine</SelectItem><SelectItem value="daily">Täglich</SelectItem><SelectItem value="weekly">Wöchentlich</SelectItem><SelectItem value="bi-weekly">Alle 2 Wochen</SelectItem><SelectItem value="monthly">Monatlich</SelectItem></SelectContent></Select><FormMessage /></FormItem> )}/>{watchRecurrence !== 'none' && (<FormField control={appointmentForm.control} name="recurrenceEndDate" render={({ field }) => ( <FormItem><FormLabel>Wiederholung endet am</FormLabel><FormControl><Input type="date" {...field} min={appointmentForm.getValues('startDate') ? appointmentForm.getValues('startDate').split('T')[0] : undefined} /></FormControl><FormMessage /></FormItem> )}/>)}</div>
                ...
                {watchVisibilityType === 'specificTeams' && (<FormField control={appointmentForm.control} name="visibleTeamIds" render={({ field }) => ( <FormItem><FormLabel>Mannschaften auswählen</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant="outline" role="combobox" className={cn("w-full justify-between", !field.value?.length && "text-muted-foreground")}>{field.value?.length ? `${field.value.length} ausgewählt` : "Mannschaften auswählen..."}</Button></FormControl></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] max-h-[--radix-popover-content-available-height] p-0"><ScrollArea className="h-48">{isLoadingGroups ? ( <div className="p-4 text-center text-sm">Lade...</div> ) : ( sortedGroupedTeams.map((group) => ( <React.Fragment key={group.id}><h4 className="font-semibold text-sm my-2 px-4">{group.name}</h4>{group.teams.map((team) => ( <div key={team.id} className="flex items-center space-x-2 px-4 py-1.5" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}><Checkbox id={`team-check-${team.id}`} checked={field.value?.includes(team.id)} onCheckedChange={(checked) => { const newValue = checked ? [...(field.value || []), team.id] : (field.value || []).filter((id) => id !== team.id); field.onChange(newValue); }} /><label htmlFor={`team-check-${team.id}`} className="font-normal w-full cursor-pointer">{team.name}</label></div> ))}</React.Fragment> )) )}</ScrollArea></PopoverContent></Popover><FormMessage /></FormItem> )}/>)}
                ...
                <FormField control={appointmentForm.control} name="locationId" render={({ field }) => ( <FormItem><div className="flex items-center justify-between"><FormLabel>Ort</FormLabel><Dialog open={isLocationDialogOpen} onOpenChange={setIsLocationDialogOpen}><DialogTrigger asChild><Button type="button" variant="outline" size="sm" className="h-7"><Plus className="h-3 w-3 mr-1" /> Verwalten</Button></DialogTrigger><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Orte verwalten</DialogTitle></DialogHeader><Form {...locationForm}><form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); }}><div className="space-y-4 py-4"><FormField control={locationForm.control} name="name" render={({ field }) => ( <FormItem><FormLabel>Name des Ortes</FormLabel><FormControl><Input placeholder="z.B. Fritz-Jacobi-Anlage" {...field} /></FormControl><FormMessage /></FormItem> )}/><FormField control={locationForm.control} name="address" render={({ field }) => ( <FormItem><FormLabel>Adresse (optional)</FormLabel><FormControl><Input placeholder="Straße, PLZ Ort" {...field} /></FormControl><FormMessage /></FormItem> )}/><Button type="button" className="w-full" onClick={locationForm.handleSubmit(onSubmitLocation)} disabled={locationForm.formState.isSubmitting}>{locationForm.formState.isSubmitting && (<Loader2 className="mr-2 h-4 w-4 animate-spin" />)}Ort Speichern</Button></div></form></Form><Separator className="my-4" /><h4 className="text-sm font-medium mb-2">Bestehende Orte</h4><ScrollArea className="h-40"><div className="space-y-2 pr-4">{locations && locations.length > 0 ? ( locations.map((loc) => ( <div key={loc.id} className="flex justify-between items-center p-2 hover:bg-accent rounded-md"><div><p>{loc.name}</p><p className="text-xs text-muted-foreground">{loc.address}</p></div><AlertDialog><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7"><Trash2 className="h-4 w-4 text-destructive" /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Sind Sie sicher?</AlertDialogTitle><AlertDialogDescription>Möchten Sie "{loc.name}" wirklich löschen?</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Abbrechen</AlertDialogCancel><AlertDialogAction onClick={() => onDeleteLocation(loc.id)} className="bg-destructive hover:bg-destructive/90">Löschen</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div> )) ) : ( <p className="text-sm text-muted-foreground text-center">Keine Orte gefunden.</p> )}</div></ScrollArea><DialogFooter className="mt-4"><DialogClose asChild><Button type="button" variant="outline">Schließen</Button></DialogClose></DialogFooter></DialogContent></Dialog></div><Select onValueChange={field.onChange} value={field.value ?? ''}><FormControl><SelectTrigger><SelectValue placeholder="Ort auswählen..." /></SelectTrigger></FormControl><SelectContent>{isLoadingLocations ? (<SelectItem value="loading" disabled>Lade...</SelectItem>) : ( locations?.map((loc) => (<SelectItem key={loc.id} value={loc.id}>{loc.name}</SelectItem>)) )}</SelectContent></Select><FormMessage /></FormItem> )}/>
      ...
      <Dialog open={isInstanceDialogOpen} onOpenChange={(open) => { setIsInstanceDialogOpen(open); if (!open) { setSelectedInstanceToEdit(null); instanceForm.reset(); } }}>
        <DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Einzelnen Termin bearbeiten</DialogTitle><DialogDescription>Ändere Details nur für diesen spezifischen Termin am{' '}{selectedInstanceToEdit?.startDate ? format(selectedInstanceToEdit.startDate.toDate(), 'dd.MM.yyyy HH:mm', { locale: de }) : ''}.</DialogDescription></DialogHeader>
          <Form {...instanceForm}><form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); }} className="space-y-4 pt-4"><input type="hidden" {...instanceForm.register('originalDateISO')} />
      ...
      <AlertDialog open={isUpdateTypeDialogOpen} onOpenChange={(open) => { if (!open) resetSingleInstanceDialogs(); setIsUpdateTypeDialogOpen(open); }}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Änderung speichern</AlertDialogTitle><AlertDialogDescription>Möchtest du diese Änderung nur für diesen Termin ({' '}{selectedInstanceToEdit?.startDate ? format(selectedInstanceToEdit.startDate.toDate(), 'dd.MM.yy', { locale: de }) : ''}) speichern oder für diesen und alle zukünftigen Termine dieser Serie?</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel onClick={resetSingleInstanceDialogs}>Abbrechen</AlertDialogCancel><Button onClick={handleSaveSingleOnly} disabled={isSubmitting}>{isSubmitting && (<Loader2 className="mr-2 h-4 w-4 animate-spin" />)}Nur diesen Termin</Button><Button onClick={handleSaveForFuture} disabled={isSubmitting}>{isSubmitting && (<Loader2 className="mr-2 h-4 w-4 animate-spin" />)}Diesen und zukünftige</Button></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Card>
        ...
        <CardContent>
          ...
          ) : ( <div className="text-center py-10 text-muted-foreground">Keine Termine entsprechen den aktuellen Filtern.</div> )}
        </CardContent>
      </Card>
    </div>
  );
