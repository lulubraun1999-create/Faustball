...
export default function AdminTerminePage() {
  ...
  const { typesMap, locationsMap, teamsMap, groupedTeams, sortedTeamsForFilter } = useMemo(() => {
    ...
    return { typesMap, locationsMap, teamsMap, groupedTeams: grouped, sortedTeamsForFilter: sortedTeams };
  ...
  const filteredAndGroupedAppointments = useMemo(() => {
    ...
    const sorted = filtered.sort((a, b) => a.instanceDate.getTime() - b.instanceDate.getTime());
  ...
  async function handleSaveForFuture() {
    ...
    try { const { firebaseApp } = initializeFirebase(); const functions = getFunctions(firebaseApp); const saveFutureInstancesFn = httpsCallable(functions, 'saveFutureAppointmentInstances'); await saveFutureInstancesFn(pendingUpdateData); toast({ title: 'Terminserie erfolgreich aufgeteilt und aktualisiert' });
  ...
  const handleEditAppointment = (appointment: UnrolledAppointment) => {
    ...
    instanceForm.reset({ originalId: appointment.originalId, originalDateISO: appointment.originalDateISO, startDate: startDateString, endDate: endDateString, isAllDay, title: titleIsDefault ? '' : appointment.title, locationId: appointment.locationId ?? '', meetingPoint: appointment.meetingPoint ?? '', meetingTime: appointment.meetingTime ?? '', description: appointment.description ?? '' });
