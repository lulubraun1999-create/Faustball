...
    } catch (error: any) { 
        console.error("Fehler beim Aufruf von saveSingleAppointmentException:", error);
        toast({ variant: "destructive", title: "Fehler beim Speichern", description: error.message || "Die Änderung konnte nicht gespeichert werden." }); 
    } finally { 
...
    if (!firestore || !pendingUpdateData || !user) return;
    setIsSubmitting(true); 
    if (!isAdmin) { toast({ variant: "destructive", title: "Fehler", description: "Nur Administratoren können diese Aktion ausführen." }); setIsSubmitting(false); return; }
    try { 
        const { firebaseApp } = initializeFirebase(); 
        const functions = getFunctions(firebaseApp); 
...
    } catch (error: any) { 
        ...
        toast({ variant: "destructive", title: "Fehler beim Speichern", description: error.message || "Die Terminserie konnte nicht aktualisiert werden." }); 
    } finally { 
        setIsSubmitting(false); 
        resetSingleInstanceDialogs(); 
    }
...
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Änderung speichern</AlertDialogTitle><AlertDialogDescription>Möchtest du diese Änderung nur für diesen Termin ({' '}{selectedInstanceToEdit?.instanceDate ? format(selectedInstanceToEdit.instanceDate, 'dd.MM.yy', { locale: de }) : ''}) speichern oder für diesen und alle zukünftigen Termine dieser Serie?</AlertDialogDescription></AlertDialogHeader>
